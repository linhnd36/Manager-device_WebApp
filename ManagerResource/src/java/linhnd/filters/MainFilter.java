...
public class MainFilter implements Filter {
    ...
    private final List<String> leaderEmp;

    public MainFilter() {
        guests = new ArrayList<>();
        guests.add("login.jsp");
        guests.add("confirmMail.jsp");
        guests.add("error.jsp");
        guests.add("signup.jsp");
        guests.add("");
        guests.add("''");
        guests.add("login");
    ...
    public void doFilter(ServletRequest request, ServletResponse response,
        ...
        if (resource.contains(".png")
            ...
            if (session == null || session.getAttribute("USER") == null) {
                ...
                if (dto.getRoleId().contains("LEADER") && leaderEmp.contains(resource)) {
                    chain.doFilter(request, response);
                } else if (dto.getRoleId().contains("MANAGER") && manager.contains(resource)) {
                    chain.doFilter(request, response);
                } else if (dto.getRoleId().contains("EMP") && leaderEmp.contains(resource)) {
                    chain.doFilter(request, response);
                } else {
                    res.sendRedirect("login.jsp");
                }
